...
if( 'Submit' == $_POST['op'] && isset($_POST['submitted_package']) ) { 
	  ...
	  $city = trim($_POST['city']);
		...
		if( 'Create Your Design' !=  $_POST['image'] ) {
				if ( $_FILES ) {
			...
			$link = "http://blacklistdir.rebelute.in/?add-to-cart=". $product_id . "&quantity=1";
?>
			...
			<form class="form-item" action="" id="primaryPostForm" method="POST" enctype="multipart/form-data" onsubmit="return validate()">
				...
				<div>
										...
										margin:10px;
											...
											margin:10px;
												...
												<div class="padl10 ginput_container ginput_container_singleproduct">
													...
													<input type="hidden" name="product_id_1" id="product_id_1" value="2077" class="ginput_quantity_4_9 gform_hidden">
													<input type="hidden" name="product_id_21" id="product_id_21" value="2078" class="ginput_quantity_4_9 gform_hidden">
													<input type="hidden" name="product_id_33" id="product_id_33" value="2079" class="ginput_quantity_4_9 gform_hidden">
													
													<input type="hidden" name="product_id_4" id="product_id_4" value="2081" class="ginput_quantity_4_9 gform_hidden">
												</div>
											</div>
											<div class="clearfix"></div>
											<div>
										...
										margin:10px;
											...
											margin:10px;
												...
												<div class="padl10 ginput_container ginput_container_singleproduct">
													...
													<input type="hidden" name="product_id_11" id="product_id_11" value="2082" class="ginput_quantity_4_9 gform_hidden">
													<input type="hidden" name="product_id_22" id="product_id_22" value="2087" class="ginput_quantity_4_9 gform_hidden">
													<input type="hidden" name="product_id_23" id="product_id_23" value="2084" class="ginput_quantity_4_9 gform_hidden">
													
													<input type="hidden" name="product_id_14" id="product_id_14" value="2086" class="ginput_quantity_4_9 gform_hidden">
												</div>
											</div>
											<div class="clearfix"></div>
											<div>
							...
							<div class="publish-ad-button" id="outside_chicago_button" style="display:none;">
									...
									<input  type="button"  class="theme_btn" id="add-to-cart-listing" style="    width: 98px !important;    padding: 10px 3px;    font-size: 14px;" value="Submit" />
...
jQuery( document ).ready(function( $ ) {
    ...
    $('#chicago_button').hide();
    $('form input[name="city"]').on('click', function() {
		 ...
		 jQuery.noConflict(); 
   		jQuery('#chicago_business input[type="radio"]').each(function(){
				this.checked = false; 
 		});
		  
		jQuery('#outside_chicago_listing input[type="radio"]').each(function(){
				this.checked = false; 
 		});
	...
	$('#weblist_chicago .ginput_container input[type="checkbox"]').on('click', function() {
		...
		if((this).checked ) {
			basic_price+= parseFloat($(this).val());
			$('#weblist_chicago .ginput_total').html();
			$('#weblist_chicago .ginput_total').html( '$ '+ basic_price);
...
function validate() {
	...
	var valid = true;
     jQuery(function(){
          var image = jQuery('input[type=radio][name=image]:checked').val();
		  if( 'Upload Image' == image ) {
				var image_name = document.getElementById('upload-images-ad').value; 
			
				if( '' == image_name ) {
					 document.getElementById('upload_error').innerHTML = "<span style='color:red;margin-left:10px;'>Please select image to upload.</span>";
					 valid = false;
...
jQuery( document ).ready(function( $ ) {
	$("#upload-images-ad").change(function(e) {
		var file, img;
		...
		if ((file = this.files[0])) {
			...
			img.onerror = function() {
				alert( "Please select valid image type ( 'jpeg, jpg, gif, png, bmp' ).");
				$("#upload-images-ad").val("");
				$("#upload_error").empty();
			};
