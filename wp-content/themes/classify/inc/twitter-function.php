...
global $redux_demo;
	...
	//include_once('../../../plugins/wp-postratings/twitter-paypal/twitteroauth.php');
	//require_once( dirname( dirname( dirname( dirname( __FILE__ )))) . '/plugins/wp-postratings/twitter-paypal/twitteroauth.php' );
    ...
    if (empty($atts['consumerkey']) || empty($atts['consumersecret']) || empty($atts['accesstoken']) || empty($atts['accesstokensecret']) || !isset($atts['cachetime']) || empty($atts['username'])) {
        return '<strong>' . __('Due to Twitter API changed you must insert Twitter APP. Check Our theme Options there you have Option for Twitter API, insert the Keys One Time', 'classify') . '</strong>';
    }
    ...
    if ($diff >= $crt || empty($jw_twitter_last_cache_time)) {
        ...
        if (!empty($tweets->errors)) {
            if ($tweets->errors[0]->message == 'Invalid or expired token') {
                return '<strong>' . $tweets->errors[0]->message . '!</strong><br />'.__('You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!','classify');
            } else {
...
if (!function_exists('shortcode_jw_twitter')) {
    ...
    function shortcode_jw_twitter($atts, $content) {
        ...
        if (is_array($jw_twitter_tweets)) {
            ...
            $output.='</ul>';
            //$output.='<div class="twitter-follow">'  . (jw_option('jw_car_follow') ? jw_option('jw_car_follow') : __('Follow Us', 'classify')) . ' - <a target="_blank" href="http://twitter.com/' . $atts['username'] . '">@' . $atts['username'] . '</a></div>';
            $output.='</div>';
            return $output;
        } else {
            return $jw_twitter_tweets;
