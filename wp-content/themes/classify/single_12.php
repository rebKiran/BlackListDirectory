...
<?php 
	$blogpost = get_post_meta($post->ID, 'blog_posts', true);	
	if ($blogpost == 'blog_posts'){
		echo "this is blog post";
	}
?>
<?php 
    	...
    	<div class="container">
	    	...
	    	<div class="span8 first">
				...
				$attachments = get_children(array('post_parent' => $post->ID,
						...
						echo "<img src='" . $image_url . "' data-big='" . $image_url . "' >";
				}		
						...
						<td>
							<div class="detail-cat">
				    			...
				    			<?php 
									...
									if ($category) {
										echo '<a href="' . get_category_link( $category[0]->term_id ) . '" title="' . sprintf( __( "View all posts in %s", "classify" ), $category[0]->name ) . '" ' . '>' . $category[0]->name.'</a> ';
									}
								?>
							</div>
							...
							if(!empty($classifyCustomFields)) {
										for ($i = 0; $i < count($classifyCustomFields); $i++) {
											...
											if($classifyCustomFields[$i][2] != 'dropdown' && $classifyCustomFields[$i][2] != 'checkbox'){
												...
												?>
												<?php if(!empty($classifyCustomFields[$i][1]) && !empty($classifyCustomFields[$i][0]) ) { ?>
												<span class="ad-detail-info">
													<span class="ad-detail">
														<?php echo $classifyCustomFields[$i][1]; ?>
													</span>
													<?php echo $classifyCustomFields[$i][0]; ?>
												</span>
										...
										<?php }  ?>	
											<!--CheckBox-->
											<?php for ($i = 0; $i < count($classifyCustomFields); $i++) {
							...
							<?php 
								$post_owner = get_post_meta($post->ID, 'owner', true); 
								if(!empty($post_owner)) {
							?>

							<span class="ad-detail-info" style="display:none;"><?php _e( 'Owner', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $post_owner; ?></span>
							</span>

							<?php } ?>	

							<?php 
								$business_role = get_post_meta($post->ID, 'business_role', true); 
								if(!empty($business_role)) {
							?>

							<span class="ad-detail-info" style="display:none;"><?php _e( 'Business Role', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $business_role; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Firstname', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $firstname; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Lastname', 'classify' ); ?> <span class="ad-detail">
							...
							<?php 
								$mailing_address = get_post_meta($post->ID, 'mailing_address', true); 
								if(!empty($mailing_address)) {
							?>

							<span class="ad-detail-info" style="display:none;"><?php _e( 'Mailing Address', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $mailing_address; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Street Address', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $street_address; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Address', 'classify' ); ?> <span class="ad-detail">
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'City', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $city; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'State', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $state; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Zipcode', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $zipcode; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Email', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $email; ?></span>
							</span>
							...
							<span class="ad-detail-info" style="display:none;"><?php _e( 'Contact', 'classify' ); ?> <span class="ad-detail">
							...
							<?php 
							 ...
							 $mylink = $wpdb->get_row( "select t.name from wp_term_relationships r INNER JOIN wp_terms t ON ( r.term_taxonomy_id = t.term_id ) WHERE r.object_id='".$post->ID ."' ", ARRAY_A );
								
								if(!empty($mylink['name'])) {
									
								//	get_the_category_by_ID( int $cat_ID )
							?>

							<span class="ad-detail-info"><?php _e( 'Business Type', 'classify' ); ?> <span class="ad-detail">
						    	<?php echo $mylink['name']; ?></span>
							</span>

							<?php } ?>	
							
							<?php 
								$business_purpose = get_post_meta($post->ID, 'business_purpose', true); 
								if(!empty($business_purpose)) {
							?>
						<span class="ad-detail-info"><?php _e( 'Description :', 'classify' ); ?>  </span>
							<span class="ad-detail-info">
						    	<?php echo $business_purpose; ?>
							</span>
							...
							<?php } ?>
							<div class="ad-detail-info" style="display:none;"><?php _e( 'Description:', 'classify' ); ?> 
								<?php echo the_content(); ?>
							</div>
							<div class="ads-tags">

								<i class="fa fa-tag"></i><span><a><?php esc_html_e( 'Tags', 'classify' ); ?></a></span><span><?php the_tags('','',''); ?></span>
								<ul class="links">
									...
									<li class="service-links-twitter-widget first">
										    <a href="https://twitter.com/share" class="twitter-share-button" data-url="<?php the_permalink(); ?>" data-lang="en"><?php esc_html_e( 'Tweet', 'classify' ); ?></a>
				...
				<div class="post-page-ad">				
	    			...
	    			<h3><?php _e( 'RELATED ADS', 'classify' ); ?></h3>
				...
				<div class="full">
						...
						<div id="contact-ad-owner-v2">
							...
							<form name="contactForm" action="<?php the_permalink(); ?>" id="contact-form" method="post" class="contactform" >
								...
								<p class="humantest"><?php _e("Human test. Please input the result of 5+3=?", "classify"); ?></p>
