...
$banner_sizes = $wpdb->get_results($querystr,ARRAY_A);
  ...
  /*$my_query = "	Select 
					...
					from 
						wp_posts
					where 
						ID IN ('2081','2082','2083','2084')";
		//	die( 'Inside');					
		$results = $wpdb->get_results($my_query,  ARRAY_A );*/
	...
	$query = "SELECT products.ID as product_id,products.post_title,postmeta.meta_value AS price
					   ...
					   JOIN {$wpdb->prefix}posts AS products ON ( products.ID = postmeta.post_id )
						...
						AND products.ID IN ('2081','2082','2083','2084')";
	...
	$query2 = "SELECT products.ID as product_id,products.post_title,postmeta.meta_value AS price
					   ...
					   JOIN {$wpdb->prefix}posts AS products ON ( products.ID = postmeta.post_id )
						WHERE postmeta.meta_key = '_sale_price'
						AND products.ID IN ('2085','2086','2087','2088')";
			...
			<div id="tab-1" >
				...
				<div>
										...
										margin:10px;
											...
											margin:10px;
												...
												<div class="padl10 ginput_container ginput_container_singleproduct">
													...
													<input type="hidden" name="product_id_11" id="product_id_11" value="<?php if(!empty($results)) { echo $results2[0]['product_id'];  } ?>" class="ginput_quantity_4_9 gform_hidden">
													<input type="hidden" name="product_id_22" id="product_id_22" value="<?php if(!empty($results)) { echo $results2[1]['product_id'];  } ?>" class="ginput_quantity_4_9 gform_hidden">
													<input type="hidden" name="product_id_23" id="product_id_23" value="<?php if(!empty($results)) { echo $results2[2]['product_id'];  } ?>" class="ginput_quantity_4_9 gform_hidden">
													
													<input type="hidden" name="product_id_14" id="product_id_14" value="<?php if(!empty($results)) { echo $results2[3]['product_id'];  } ?>" class="ginput_quantity_4_9 gform_hidden">
												</div>
											</div>
											<div class="clearfix"></div>
											<div>
							...
							<div class="publish-ad-button" id="chicago_button">
									...
									<button class="theme_btn" id="edit-submit" name="op" value="Publish Ad" type="submit" style="margin-left: 20px;margin-top: 20px;"><?php _e('Submit', 'classify') ?></button>
							</div>
							<div style="clear:both"></div>
							<br>
							<div class="publish-ad-button hide" id="outside_chicago_button">
								<input type="hidden" name="submitted_package" id="submitted_package" value="true" />
									
									<input name="add-to-cart" id="add-to-cart" type="hidden" value="<?php if(!empty($results)) { echo $$results2[0]['product_id'];  } ?>" />
									<input name="quantity" type="hidden" value="1" min="1"   />
									<button class="button-secondary theme_btn" id="add-to-cart_product" style="    width: 98px !important;    padding: 10px 3px;    font-size: 14px;"><?php _e('Submit', 'classify') ?></button>
