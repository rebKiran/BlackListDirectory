...
class Bc_Templates {	
      protected $post_edit_id = 0;

	public function __construct() 
	{	
               if ( !is_admin() ) {  
                     $arrRequestUrl = explode( '/', trim($_SERVER['REQUEST_URI'],  '/'));

		     $cntRequestParam = count($arrRequestUrl);
		     
                     if( '2' == $cntRequestParam ) {
			 $this->post_edit_id = $arrRequestUrl['1'];
		     } else {
			$this->post_edit_id = 0;
		     }	
                }
	...
	{ 
		...
		if ( !is_admin() ) { 
			global $wpdb;
                ...
                $directory_listing = $wpdb->get_row( "SELECT dl.product_id, pb.size FROM  wp_directory_listing dl join wp_product_banner pb ON ( dl.product_id = pb.product_id )
								WHERE 
									dl.questionnaire_id= '".$_SESSION['questionnaire_id']."' 
								 ", ARRAY_A );

                     $canvasLength = explode('*', $directory_listing['size'] );

                     $canvasHt = intval($canvasLength['0']);
                     $canvasWd = intval($canvasLength['1']);
                    }
		...
		$html = '';
               $html = '<div  class="overflow-div" style="width: 1008px;">';
		$html.= '<div class="pro_ads_banner_creator span7" style="width: 663px;">';
            $html.= '<div id="bc_canvas_holder">';
                $html.= '<canvas id="bcCanvas" height="'. $canvasHt.'" width="'. $canvasWd.'" >'.__('Canvas not suported','wpproads').'</canvas>';
            $html.= '</div>';
            
            $html.= '<div id="button_tools_cont" style="margin: 10px 0 0px 0;"> ';
...
if ( is_admin() ) { 
            ...
            $html.= '<div id="button_tools_cont" style="margin: -2px 0 10px 0;"></div>';
           
            $html.= '<ul id="log" style="max-height: 130px;overflow-x: hidden;overflow-y: auto;"></ul>';
	...
	if ( !is_admin() ) { 
            $html.= '<input type="hidden" id="post_edit" name="post_edit" value="'.$this->post_edit_id.'" />';
        }
	...
	public function banner_creator_sidebar_area( $args = array() )
	{
		...
		$html = '';
		$html.= '<div class="sidebar vbc_sidebar" style="width: 330px;text-align:left">';
            $html.= '<div id="widgets-right" class="pro_ads_banner_creator">';
			
				$html.= '<div class="menu_cont">';
	
					$html.= '<div class="menu_icons">';
						/* $html.= '<a class="vbc_tooltip" title="Banner Size" data-target="vbc_banner_size"><i class="fa fa-expand"></i></a>'; */

                                                $html.= '<a class="vbc_tooltip selected" title="Add Text" data-target="vbc_add_text"><i class="fa fa-font"></i></a>';
					...
					$html.= '<div class="menu_options">';
						$html.= '<div class="vbc_banner_size vbc_menu_option">';
						...
						$html.= '</div>';
						$html.= '<div class="vbc_add_text  vbc_menu_option" style="display:inline-block;">';
				...
				$html.= '</div>';
				if(empty($this->post_edit_id)) {
					 $html.= '<div style="margin-left:0px;padding-top:5px;"><input name="add-to-cart" id="add-to-cart" type="hidden" value="" /><input name="quantity" type="hidden" value="1" min="1"  />
					<a class="theme_btn" id="add-to-cart_product" class="create_cart">'.__('Next','wpproads').'</a></div>'; 
				} else {
					$html.= '<div style="margin-left:0px;padding-top:5px;">
					<a class="button-secondary theme_btn" id="add-to-cart_product" class="create_cart">'.__('Submit','wpproads').'</a></div>';
				} 	
			$html.= '</div>';
			$html.= '</div>';
$html.= '</div>';
		...
		$html.= '<div>';
			$html.= '<div id="sidebar" >';
				...
				$html.= '<div class="sidebar-description" style="padding:10px; background:#EFEFEF; position:relative;">';
					...
					$html.= '<div class="dropdown-custom" style="margin: 4px;">';
						$html.= '<div><small>'.__('Font','wpproads').'</small></div>';
