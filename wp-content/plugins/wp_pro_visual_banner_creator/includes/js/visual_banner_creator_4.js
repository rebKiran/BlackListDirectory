...
function bc_save_image_banner(){
	...
	jQuery.ajax({
	   type: "POST",
	   url: ajaxurl,
	   data: "action=bc_save_banner_img&banner_id="+banner_id+"&price="+price+"&postTitle="+postTitle+"&data_uri="+ data_uri+"&json="+json,
	   success: function( obj ){  
		  // jQuery('#image_save_id').val(1);
		  // jQuery('input[type="hidden"][name="add-to-cart"]').val(banner_id);
		  // jQuery('.cart .single_add_to_cart_button').show();
...
jQuery(document).ready(function($){
	...
	*/
	...
	$('#banner_size').click(function(){
		...
		if( size ){
		//	alert('banner_size : '+size+' banner_id : '+ banner_id +' ajaxurl : '+ajaxurl);
			$.ajax({
			   type: "POST",
			   url: ajaxurl,
			   data: "action=bc_show_banner_price&banner_id="+banner_id+"&banner_size="+size,
			   
			  // data: "action=bc_show_banner_price&banner_id="+banner_id,
			   success: function( obj ){ 
	...
	$('#add-to-listing').click(function(){
		...
		bc_save_image_banner();
			...
			window.location =  'http://localhost/blacklist_project/about-your-business/';
		...
		canvas.deactivateAllWithDispatch().renderAll();
			...
			if( product_id ){
				$.ajax({
				   type: "POST",
				   url: ajaxurl,
				   data: "action=bc_add_to_cart&id="+product_id+"&banner_id="+banner_id+"&price="+product_price+"&data_uri="+ data_uri+"&json="+json+'&banner_url='+banner_url,
				   success: function( obj ){
					   //alert(obj)
				   }
				});
