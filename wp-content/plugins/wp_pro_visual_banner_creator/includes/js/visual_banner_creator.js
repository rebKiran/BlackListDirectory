	...
	*/
	...
	$('#banner_size').click(function(){
		...
		if( size ) {
			...
			} else if (size == '360x240') {
			   product_id = '2071';
			   price = '50.00';

			} else if (size == '430x140') {
			   product_id = '2073';
			   price = '35.00';

			} else if (size == '470x250') {
			   product_id = '2074';
			   price = '45.00'
			}
			
                        $('#banner_price').show();
			$('#banner_price').html( 'Price :-   <span class="woocommerce-Price-currencySymbol">$</span> ' + price);
					  
			$('#add-to-cart').val( product_id );
		}
             }	
	...
	$('#add-to-cart_product').click(function(){
		...
		var product_price = jQuery('#hid_banner_price').val();
		var banner_id = jQuery('#banner_id').val();
		var post_edit_id = jQuery('#post_edit').val();
       ...
       jQuery.ajax({
        type: "POST",
	url: ajaxurl, 
	data: "action=bc_save_banner_img&banner_id="+banner_id+"&data_uri="+data_uri+"&json="+json+"&post_edit_id="+post_edit_id,
	...
	success: function( obj ){  
				  ...
				  var link_url = "http://blacklistdir.rebelute.in/success";
