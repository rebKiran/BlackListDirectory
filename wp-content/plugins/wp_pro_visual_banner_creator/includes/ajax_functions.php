...
function bc_save_banner_img_callback() 
{
	...
	$post_edit_id 	= !empty($_POST['post_edit_id']) ? $_POST['post_edit_id'] : '';
	$data_uri = !empty($_POST['data_uri']) ? $_POST['data_uri'] : '';
	$json = !empty($_POST['json']) ? base64_encode(stripslashes($_POST['json'])) : '';
	$banner_id = !empty($_POST['banner_id']) ? $_POST['banner_id']: '';
		...
		$_SESSION['questionnaire_id']['banner_id'] = $banner_id; 
		$_SESSION['questionnaire_id']['data_uri'] = $data_uri; 
		$_SESSION['questionnaire_id']['json'] = $json; 
	...
	$filter = apply_filters( 'vbc_upload_banner', $upload_dir['url'], $filename, $_POST['banner_id'] );
       ...
       exit;
		...
		$attachment_id = get_post_meta( $post_edit_id, '_thumbnail_id', true );
		//wp_delete_attachment( $attachment_id );
		
		$arguments = array(
					'numberposts'   => -1,
					'post_type'     => 'attachment',
					'post_parent'   => $post_edit_id,
					'post_status'   => null,
					'orderby'       => 'menu_order',
					'order'         => 'ASC'
		);
	   ...
	   $curent_thumb=get_post_thumbnail_id($post_edit_id);
	...
	// Insert the attachment.
		$attach_id = wp_insert_attachment( $attachment, $upload_dir['path'].'/'.$filename );
		 ...
		 add_post_meta($post_edit_id, '_thumbnail_id', $attach_id );
		// Make sure that this file is included, as wp_generate_attachment_metadata() depends on it.
		require_once( ABSPATH . 'wp-admin/includes/image.php' );
