...
function get_ecwmv_events( $atts ) {
	...
	if($view == 'list'){
		...
		if($event_loop->have_posts()){
			...
			$html .= 		'<a href="'.$permalink.'"><img class="single-thumb" src="'.$url.'" title="'.$title.'" ></a>';
			$html .= 	'</div>';
			...
			$html .= '<h4 style="text-align:center;"> "Currently No Upcoming Events".  Please Check Back.</h4>';
		}
		...
		if($event_loop->have_posts()){
			...
			$html .= '<a href="'.$permalink.'"><img class="grid-thumb" src="'.$url.'" title="'.$title.'"></a>';
			$html .= '<h3><a href="'.$permalink.'">'.$title.'</a></h3>';
			$html .= '<p><i class="fa fa-map-marker"></i> '.$location.'</p>';
			$html .= '<p><i class="fa fa-calendar"></i> '.$start_time.' @ '.date('j M Y',strtotime($start_date)).'</p>';
			if(!empty($org_phone)){
		...
		if($event_loop->found_posts > 0 && $event_loop->have_posts()){
			while ($event_loop->have_posts()): $event_loop->the_post();
