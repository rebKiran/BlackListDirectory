...
//        $shop_currency_symbol=get_woocommerce_currency_symbol();
        ...
        $product_metas = get_proper_value($wpc_metas, $editor->item_id, array());

        $canvas_w = $editor->wpd_product->get_option($product_metas, $general_options, "canvas-w", 800);
        $canvas_h = $editor->wpd_product->get_option($product_metas, $general_options, "canvas-h", 500);
        $watermark = get_proper_value($product_metas, "watermark", "");
        $cmyk = $editor->wpd_product->get_option($product_metas["output-settings"], $wpc_output_options, "wpc-cmyk-conversion", "no");
        $output_format="png";
        if($cmyk=="yes")
            $output_format="jpg";

        $bounding_data = get_proper_value($product_metas, 'bounding_box', array());
        ...
        $output_w = $editor->wpd_product->get_option($wpc_output_product_settings, $wpc_output_options, "wpc-min-output-width", $canvas_w);
        $output_loop_delay = $editor->wpd_product->get_option($wpc_output_product_settings, $wpc_output_options, "wpc-output-loop-delay", 1000);
