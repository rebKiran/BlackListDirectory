...
//            "url" => $url,
    ...
    function add_custom_design_to_cart_ajax() {
		...
		WC()->cart->empty_cart();
		unset($_SESSION['wpc_generated_data']);
		unset($_SESSION['wpc_design_pricing_options']);
        $message = "";
        ...
        if (wp_mkdir_p($generation_path)) {
            ...
            if (!empty($result) && is_array($result)) {
				...
				$attachment = array(
					'guid'           => $generation_url, 
					'post_mime_type' => $_FILES['part1']['type']['image'],
					'post_title'     => preg_replace( '/\.[^.]+$/', '', basename( $upload_dir['path'].'/'.$tmp_dir ) ),
					'post_content'   => '',
					'post_status'    => 'inherit'
				);
                
                // Insert the attachment.
				$attach_id = wp_insert_attachment( $attachment, $generation_url );
				
				// Make sure that this file is included, as wp_generate_attachment_metadata() depends on it.
				require_once( ABSPATH . 'wp-admin/includes/image.php' );
		
				// Generate the metadata for the attachment, and update the database record.
				$attach_data = wp_generate_attachment_metadata( $attach_id, $generation_url );
				...
				wp_update_attachment_metadata( $attach_id, $attach_data );
					...
					$wpdb->query("UPDATE 
									...
									SET 
										post_id='".$attach_id."',
																		image='". $upload_dir['path'].'/'.$tmp_dir ."',
																		banner_url= '". basename( $upload_dir['path'].'/'.$tmp_dir ) ."'
                ...
                $final_canvas_parts["output"]["tpl"] = $_POST["tpl"];
...
//                      $quantity=$variations_data[1];
						...
						// This will be your custome price  
							/*foreach (  $woocommerce->cart->cart_contents as $key => $value ) {
								$value['data']->price = $_SESSION['banner_price'];
							}*/
	    ...
	    echo json_encode(array("success" => $newly_added_cart_item_key,
            ...
            "url" => $cart_url,
            "attach_id" => $attach_id,
            "image_path" => basename( $upload_dir['path'].'/'.$tmp_dir )
        ));
        die();
    }
...
//        var_dump($item);
    ...
    function get_user_account_products_meta_new($item_id, $item, $order) {
        GLOBAL $wpd_settings;
        $options = $wpd_settings['wpc-general-options'];
//        var_dump($item);
        $output = "";
        ...
        if (!in_array($order->post_status, $invalid_statuses) && $download_btn !== "0" && isset($item["variation_id"]) && (!empty($item["variation_id"]) || $item["variation_id"] == "0")) {
            $product = wc_get_product($item["variation_id"]);
            $item_id = uniqid();
            //        var_dump($product);
            ob_start();
...
//                }
    ...
    function get_design_price_ajax() {
		...
		$_SESSION['banner_price'] = '';
		$_SESSION['product'] = '';
		if( empty( $_SESSION['banner_price'] ) ) {
		     	$_SESSION['banner_price'] = $results[$variation_id];
		}
		if( empty( $_SESSION['product'] ) ) {
		     	$_SESSION['product'] = $variation_id;
		}
		
        echo json_encode(array(
            "prices" => $results,
			"price" => $_SESSION['banner_price'],
			"variation_id" => (!empty( $_SESSION['banner_price'] )) ? $_SESSION['banner_price'] : $results[$variation_id],
			"proiduct" => $variation_id
                )
        );
        die();
    }
...
//                $id_to_check = $product->variation_id;
    ...
    public function get_additional_price($product_id, $data) {
		...
		/*print_r($elements_analysis); 
			print_r($priceable_elements); 
			die( 'Here' );*/
            ...
            foreach ($data as $part_data) {
                ...
                foreach ($decoded_json->objects as $object) {
                    ...
                    if (!isset($elements[$object_type]) || !is_array($elements[$object_type])) {
						...
						if($object->type == "i-text") {$elements['name'] = array();$elements['font'] = array();}
                    }
                    ...
                    if ($object_type == "text" || $object_type == "i-text") {
						...
						$input_text = $object->text;
						
						if( ctype_upper( $input_text ) ) { 
							 array_push($elements['name'], array( "price" => $price ));
						}
						
						if( '000000' !==  trim($_POST['color']) ) { 
							
							 array_push($elements['font'], array( "price" => $price ));
						}						
    ...
    private function fix_total_by_types($elements_by_types) {
        foreach ($elements_by_types as $type => $elements) {
            foreach ($elements as $i => $element) {
				...
				else if (isset($elements_by_types[$type][$i]["name_in_all_caps"]))
                    $elements_by_types[$type][$i]["name_in_all_caps"] = count($elements);
				else if (isset($elements_by_types[$type][$i]["font_color"]))
                    $elements_by_types[$type][$i]["font_color"] = count($elements);
            }
        }
    ...
    private function get_group_results($priceable_elements, $rules) {
        ...
        foreach ($rules as $rule_arr) {
            ...
            //else if it's a vector rule
            ...
            } else if ($this->wpc_starts_with($rule_arr["param"], "name")) {
				
                if (isset($priceable_elements["name"]) && !empty( $priceable_elements["name"] )) {
                   // $results_arr = $this->wpc_check_rule($priceable_elements["name"], $rule_arr);
				    $results_arr = array( 1 );
				}	
                else
                    $results_arr = array(false);
				
                if (!isset($group_results["name"]))
                    $group_results["name"] = array();
                //            var_dump($results_arr);
                array_push($group_results["name"], $results_arr);
				
            } else if ($this->wpc_starts_with($rule_arr["param"], "font")) {
				
                if (isset($priceable_elements["font"]) && !empty( $priceable_elements["font"] )) {
                   // $results_arr = $this->wpc_check_rule($priceable_elements["name"], $rule_arr);
				    $results_arr = array( 1 );
				}	
                else
                ...
                if (!isset($group_results["font"]))
                    $group_results["font"] = array();
                //            var_dump($results_arr);
                array_push($group_results["font"], $results_arr);
            }
...
//            var_dump($pdf_format);
        ...
        {
            $pdf_width = $wpd_product->get_option($variation_output_settings, $global_output_settings, "pdf-width", 0);
            $pdf_height = $wpd_product->get_option($variation_output_settings, $global_output_settings, "pdf-height", 0);
            $pdf_unit = $wpd_product->get_option($variation_output_settings, $global_output_settings, "pdf-unit", "in");
...
//        $pagHeight = $pdf->getPageHeight();
    ...
    private function export_data_to_files($generation_dir, $data, $variation_id, $zip_name, $pdf_watermark = false) {
        ...
        $wpc_cmyk_conversion=$wpd_product->get_option($product_output_settings, $wpc_output_options, "wpc-cmyk-conversion", "no");
        if(!class_exists("Imagick"))
            $wpc_cmyk_conversion="no";
//        var_dump($cmyk_ouput);
//        if (isset($wpc_output_options['wpc-cmyk-conversion']) && class_exists("Imagick"))
                    ...
                    foreach ($_FILES["layers"]["tmp_name"][$part_key] as $layer_data) {
                        ...
                        if (move_uploaded_file($layer_data, $output_file_path)) {
                            if ($pdf_watermark && $watermark_id) {
                                $watermarked_layer_name = uniqid("watermarked_layer_") . ".$wpc_img_format";
                            ...
                            if ($wpc_cmyk_conversion == "yes") {
                                $wpc_cmyk_profil = get_proper_value($wpc_output_options, "wpc-cmyk-profil", false);
                                if ($wpc_cmyk_profil) {
