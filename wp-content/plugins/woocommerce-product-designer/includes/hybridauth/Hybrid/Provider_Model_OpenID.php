...
/**
 * To implement an OpenID based service provider, O_Provider_Model_OpenID
 * can be used to save the hassle of the authentication flow. 
 * 
 * Each class that inherit from O_Provider_Model_OAuth2 have only to define
 * the provider identifier : <code>public $openidIdentifier = ""; </code>
 *
 * O_Provider_Model_OpenID use LightOpenID lib which can be found on
 * Hybrid/thirdparty/OpenID/LightOpenID.php
 */
class O_Provider_Model_OpenID extends O_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once O_Auth::$config["path_libraries"] . "OpenID/LightOpenID.php"; 
		
		// An error was occurring when proxy wasn't set. Not sure where proxy was meant to be set/initialized.
		O_Auth::$config['proxy'] = isset(O_Auth::$config['proxy'])?O_Auth::$config['proxy']:'';
		
		$this->api = new LightOpenID( parse_url( O_Auth::$config["base_url"], PHP_URL_HOST), O_Auth::$config["proxy"] ); 
	}
	...
	*/
	function loginBegin()
	{
		...
		O_Auth::redirect( $this->api->authUrl() );
	}
	...
	*/
	function loginFinish()
	{
		...
		O_Auth::storage()->set( "hauth_session.{$this->providerId}.user", $this->user );
	}
	...
	*/
	function getUserProfile()
	{
		...
		$this->user = O_Auth::storage()->get( "hauth_session.{$this->providerId}.user" ) ;
