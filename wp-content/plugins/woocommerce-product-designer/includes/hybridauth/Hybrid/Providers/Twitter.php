...
/**
* O_Providers_Twitter provider adapter based on OAuth1 protocol
*/
class O_Providers_Twitter extends O_Provider_Model_OAuth1
{
	/**
 	...
 	function loginBegin()
 	{
		...
		// redirect the user to the provider authentication url with force_login
 		if ( isset( $this->config['force_login'] ) && $this->config['force_login'] ){
 			O_Auth::redirect( $this->api->authorizeUrl( $tokens, array( 'force_login' => true ) ) );
 		}
