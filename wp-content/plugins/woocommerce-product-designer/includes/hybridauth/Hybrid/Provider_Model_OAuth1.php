...
/**
 * To implement an OAuth 1 based service provider, O_Provider_Model_OAuth1
 * can be used to save the hassle of the authentication flow. 
 * 
 * Each class that inherit from O_Provider_Model_OAuth1 have to implemenent
 * at least 2 methods:
 *   O_Providers_{provider_name}::initialize()     to setup the provider api end-points urls
 *   O_Providers_{provider_name}::getUserProfile() to grab the user profile
 *
 * O_Provider_Model_OAuth1 use OAuth1Client v0.1 which can be found on
 * Hybrid/thirdparty/OAuth/OAuth1Client.php
 */
class O_Provider_Model_OAuth1 extends O_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once O_Auth::$config["path_libraries"] . "OAuth/OAuth.php";
		require_once O_Auth::$config["path_libraries"] . "OAuth/OAuth1Client.php";
		...
		if( isset( O_Auth::$config["proxy"] ) ){
			$this->api->curl_proxy = O_Auth::$config["proxy"];
		}
	}
	...
	*/
	function loginBegin()
	{
		...
		O_Auth::redirect( $this->api->authorizeUrl( $tokens ) );
	}
