...
/**
 * O_Provider_Adapter is the basic class which O_Auth will use
 * to connect users to a given provider. 
 * 
 * Basically O_Provider_Adapterwill create a bridge from your php 
 * application to the provider api.
 * 
 * O_Auth will automatically load O_Provider_Adapter and create
 * an instance of it for each authenticated provider.
 */
class O_Provider_Adapter
{
	...
	*/
	...
	{
		O_Logger::info( "Enter O_Provider_Adapter::factory( $id )" );
		...
		if( isset( $this->config["wrapper"] ) && is_array( $this->config["wrapper"] ) ){
			...
			require_once O_Auth::$config["path_providers"] . $this->id . ".php" ;

			$this->wrapper = "O_Providers_" . $this->id; 
		}
	...
	/**
	* O_Provider_Adapter::login(), prepare the user session and the authentication request
	* for index.php
	*/
	function login()
	{
		O_Logger::info( "Enter O_Provider_Adapter::login( {$this->id} ) " );

		if( ! $this->adapter ){
			throw new Exception( "O_Provider_Adapter::login() should not directly used." );
		}

		// clear all unneeded params
		foreach( O_Auth::$config["providers"] as $idpid => $params ){
			O_Auth::storage()->delete( "hauth_session.{$idpid}.hauth_return_to"    );
			O_Auth::storage()->delete( "hauth_session.{$idpid}.hauth_endpoint"     );
			O_Auth::storage()->delete( "hauth_session.{$idpid}.id_provider_params" );
		}
		...
		$O_AUTH_URL_BASE = O_Auth::$config["base_url"];
		...
		$this->params["login_start"] = $O_AUTH_URL_BASE . ( strpos( $O_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.start={$this->id}&hauth.time={$this->params["hauth_time"]}";

		# for default HybridAuth endpoint url hauth_login_done_url
		# 	auth.done   required  the IDp ID
		$this->params["login_done"]  = $O_AUTH_URL_BASE . ( strpos( $O_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.done={$this->id}";

		O_Auth::storage()->set( "hauth_session.{$this->id}.hauth_return_to"    , $this->params["hauth_return_to"] );
		O_Auth::storage()->set( "hauth_session.{$this->id}.hauth_endpoint"     , $this->params["login_done"] ); 
		O_Auth::storage()->set( "hauth_session.{$this->id}.id_provider_params" , $this->params );

		// store config to be used by the end point 
		O_Auth::storage()->config( "CONFIG", O_Auth::$config );

		// move on
		O_Logger::debug( "O_Provider_Adapter::login( {$this->id} ), redirect the user to login_start URL." );

		O_Auth::redirect( $this->params["login_start"] );
	}
	...
	*/ 
	...
	{
		O_Logger::info( "Enter O_Provider_Adapter::$name(), Provider: {$this->id}" );
		...
		if ( ! method_exists( $this->adapter, $name ) ){
			throw new Exception( "Call to undefined function O_Providers_{$this->id}::$name()." );
		}
	...
	*/
	public function getAccessToken()
	{
		if( ! $this->adapter->isUserConnected() ){
			O_Logger::error( "User not connected to the provider." );
	...
	*/
	...
	{ 
		...
		$callback_url = O_Auth::storage()->get( "hauth_session.{$this->id}.hauth_return_to" );

		// remove some unneed'd stored data 
		O_Auth::storage()->delete( "hauth_session.{$this->id}.hauth_return_to"    );
		O_Auth::storage()->delete( "hauth_session.{$this->id}.hauth_endpoint"     );
		O_Auth::storage()->delete( "hauth_session.{$this->id}.id_provider_params" );

		// back to home
		O_Auth::redirect( $callback_url );
	}
	...
	*/
	...
	{ 
		if( isset( O_Auth::$config["providers"][$id] ) ){
			return O_Auth::$config["providers"][$id];
		}
