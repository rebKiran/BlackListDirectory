...
//            return false;
            ...
            if (wpd.clip_w && wpd.clip_h && wpd.clip_w > 0 && wpd.clip_h > 0 && wpd.clip_type == "rect")
            {
                ...
                wpd_editor.canvas.clipTo = function (ctx) {
                    ...
                    {
                        if (wpd.clip_rr > 0)
                            roundRect(ctx, (clip_x), (clip_y), parseFloat(wpd.clip_w * ratio), parseFloat(wpd.clip_h * ratio), parseFloat(wpd.clip_rr * ratio), "", wpd.clip_border);
                        else
                        {
                            ctx.rect(clip_x, clip_y, wpd.clip_w * ratio, wpd.clip_h * ratio);
                            if (wpd.clip_border)
                            {
            ...
            else if (wpd.clip_r && wpd.clip_r > 0 && wpd.clip_type == "arc")
            {
                ...
                if (wpd.clip_x)
                    clip_x = wpd.clip_x * ratio;
                var clip_y = optimal_dimensions[1] - wpd.clip_h * ratio / 2;
                if (wpd.clip_y)
                    clip_y = wpd.clip_y;

                wpd_editor.canvas.clipTo = function (ctx) {
                    ctx.arc(clip_x, clip_y, wpd.clip_r * ratio, 0, 2 * Math.PI);
...
//                        open_src_panel(options);
            ...
            wpd_editor.canvas.on('object:modified', function (options) {
				...
				var color_style = $('#txt-color-selector').attr('style');		
				var arrColor = color_style.split(':');						
				var rgb = arrColor[1].match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i);
...
//            console.log(img_src);
            var img_src = obj.src;
//            console.log(img_src);
            var in_cliparts = $("#wpd-cliparts-wrapper img[src='" + img_src + "']").length;
            var in_facebook = $("#facebook-panel img[src='" + img_src + "']").length;
            var in_instagram = $("#instagram-panel img[src='" + img_src + "']").length;
...
//            console.log(variations);
			...
			alert( 'color_style :' + color_style );
			
			var arrColor = color_style.split(':');						
			var rgb = arrColor[1].match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i);
			var hex =  (rgb && rgb.length === 4) ? "#" + ("0" + parseInt(rgb[1],10).toString(16)).slice(-2) + ("0" + parseInt(rgb[2],10).toString(16)).slice(-2) + ("0" + parseInt(rgb[3],10).toString(16)).slice(-2) : '';*/
			
			var style_color = $('#txt-color-selector').attr('style');	
			var arrColor = style_color.split(':');
			
			var strColorCode = 	arrColor[1];	
			var hex =  rgb2hex(strColorCode);
			var hex = hex.replace('#','');
			var hex = hex.replace(';','');
                ...
                $.each(canvas_objs, function (obj_index, obj_value) {
                    if (obj_value.type == 'text') {
                        ...
                        $.each(text_prop_array, function (prop_index, prop_name) {
							...
							/*if( 'textBackgroundColor' == prop_name && '000000' != obj_value.get(prop_name ) {
...
//            console.log(wpd.output_loop_delay);
                                    ...
                                    $.ajax({
                                        ...
                                        {  
                                            ...
                                            {  
                                                 ...
                                                 /*$(location).attr('href', response.url);*/
                                                 var link_url = "http://blacklistdir.rebelute.in/website-listing-options/";
		 ...
		 //Add to cart
        $(document).on("touchstart click", "#back-btn", function ()
        {
			var link_url = "http://blacklistdir.rebelute.in/member-packages/";
			window.location =  link_url; 
		});	
		
		/*$(document).on("click", "#txt-color-selector", function ()
        {   
